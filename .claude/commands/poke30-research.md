...
## 鉄則（最重要）
- **状態を必ず区別する。** `status` は `'open'`（受付中）/ `'scheduled'`（受付予定）/ `'ended'`（受付終了）。
  ...
  - `ended`：受付が終わったもの。**削除せず ended にして記録に残す**（「この店は抽選をやった」と分かるように）。確定した実施期間を `applyStart`/`applyEnd` に入れる。
  - 状態遷移：`scheduled`→（受付開始）→`open`→（受付終了）→`ended`。受付が締め切られたら open/scheduled から `ended` に更新する。
